...
class apb_base_seq extends uvm_sequence #(apb_seq_item);
    ...
    task do_write(bit [7:0] addr, bit [31:0] data);
        ...
        if (!item.randomize() with {
            ...
            `uvm_fatal(get_type_name(), "do_write() Ramdomization Fail!")
        finish_item(item);
    ...
    task do_read(bit [7:0] addr, bit [31:0] rdata);
        apb_seq_item item;
        item = apb_seq_item::type_id::create("item");
        start_item(item);
        if (!item.randomize() with {
        ...
        finish_item(item);
        `uvm_info(get_type_name(), $sformatf("do_read() 전송 완료: addr=0x%02h, rdata=0x%08h",

...
//-- -- -- -- -- -- -- -- -- -- -- -myself code-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -void fnd_display(int counter, int step, uint32_t* fnd_digit);
void init();
int main() {
    init();
    ...
    while (1) {
        ...
        for (int i = 0; i < 4; i++) {
            fnd_display(counter / 100, i, &fnd_digit);
...
void fnd_display(int counter, int step, uint32_t* fnd_digit) {
    int num = 0;
    uint32_t fnd_data = 0;
    ...
    if (*fnd_digit == 0b1101) fnd_data = fnd_data - 0x80;
    GPIOB->ODR = fnd_data;
    GPIOC->ODR = *fnd_digit;
}

void init() {
    GPIOA->CR = 0x00;  // btn[0] = run /btn[1] = stop/btn[2] = clear
    GPIOB->CR = 0xff;  // data_data[7:0]
    GPIOC->CR = 0xff;  // data_digit[3:0]
}
